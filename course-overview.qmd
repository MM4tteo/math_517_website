...
- add informations about the course organization from lecture 1
- 
:::
...
Submissions will (mostly) be HTML documents and corresponding R Markdown files (and possibly R scripts) available on students' GitHub repos. Those files must provide:
...
* reference all work of someone else (even when e.g. copying and modifying code from Stack Overflow)
* discussion summarizing what you have achieved and what additional questions and/or future work your analysis spawns

The above pertains to projects. For assignments, adjust accordingly. For example, output of an assignment may be only a single plot - in that case just describe the plot.
...
Now, in order to obtain points for your assignment, do the following:

* solve the assignment and push your solution to your GitHub repo.
* make sure your repo is well organized, so the teachers can orient themselves in it well.
* provide a link to your `.html` file (or whatever file is your solution) to Moodle, in the corresponding Assignment.
  - this link does not have to render an HTML (may only show a code in your GitHub repo), it is just for cross-checking
  - make this link clickable (by `ctrl+K`)
  
You will receive grades in the Moodle and, if appropriate, feedback either via Moodle or via *issue reporting* on GitHub.
  
## Projects

The workflow above is mostly for the assignments, since the projects require one more ingredient: collaboration. That can either be done locally, synchronizing pushes to the respective repositories on GitHub (which we recommend for the small project), or by actually utilizing the collaborative features of GitHub by working in a single repo and using tools such as *branching*, *forking* and *pull requests* to move forward. For the final project, we would actually suggest something intermediate:
...
* the one student goes to the repo, into settings -> collaborators and adds the other group members, which gives them the right to push into the repo
  - there is also no problem with sharing your "StatComp-[SCIPER]" repo with your collaborators, if you wish to do so instead, so you may opt not to create a new repo
  - if you create a new repo, no need to invite the teachers into it
...
* when the project is finished, every student pushes it to the private "StatComp-[SCIPER]" repo and submits as described above
  - In case of a group of size 3, submit right under your link the distribution of work like described [above](#groups).
