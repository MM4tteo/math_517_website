...
$$
CV(\widehat{m}) := \frac{1}{N}\sum_{n=1}^N \big\lbrace y_n - \widehat{m}^{(-n)}(x_n) \big\rbrace^2,
$$
...
1. Artificially turn the unsupervised problem into a supervised one:
    ...
    * Define
    $$
    Err_k(r) = \sum_{n \in J_k} \| \mathbf (\mathbf x_n^{obs},\mathbf x_n^{miss})^\top - (\mathbf x_n^{obs},\widehat{\mathbf x}_n^{miss})^\top \|_2^2
    $$
